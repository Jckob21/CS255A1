...
public class test extends Application {
	...
	public void ReadData() throws IOException {
		...
		// couldn't find the equivalent in Java)
		...
		for (k = 0; k < 113; k++) {
			for (j = 0; j < 256; j++) {
				for (i = 0; i < 256; i++) {
					...
					// swapped
					b1 = ((int) in.readByte()) & 0xff; // the 0xff is because Java does not have unsigned types (C++ is
														// so much easier!)
					b2 = ((int) in.readByte()) & 0xff; // the 0xff is because Java does not have unsigned types (C++ is
														// so much easier!)
		...
		// (i.e. there are 3366 levels of grey, and now we will normalise them to 0-1
		// for display purposes
		// I know the min and max already, so I could have put the normalisation in the
		// above loop, but I put it separate here
		for (k = 0; k < 113; k++) {
			for (j = 0; j < 256; j++) {
				for (i = 0; i < 256; i++) {
	...
	public void ThumbWindow(double atX, double atY) {
...

		{
			...
			for (int x = 0; x < thumb_image.getHeight(); x++) {
				for (int y = 0; y < thumb_image.getWidth(); y++) {
					
					image_writer.setColor(x, y, color);
				}
			}
			//TODO test code - to be upgraded
			//6.7368
//			for(int y = 0; y < 38; y++) {
//				for(int x = 0; x < 38; x++) {
//					int relativeX = (int) (x*6.7368);
//					int relativeY = (int) (y*6.7368); 
//					image_writer.setColor(y, x, Color.color(grey[76][relativeX][relativeY], grey[76][relativeX][relativeY], grey[76][relativeX][relativeY]));
//				}
//			}
				...
				for(int col = 0; col < 12; col++) {
					for(int y = 0; y < 38; y++) {
						for(int x = 0; x < 38; x++) {
							...
							if(pictureNo < 113) {
								image_writer.setColor(y + col*(38+4), x + row*(38+4),
										Color.color(grey[pictureNo][relativeX][relativeY], grey[pictureNo][relativeX][relativeY], grey[pictureNo][relativeX][relativeY]));
							}
		...
		// Add mouse over handler - the large image is change to the image the mouse is
		// over
